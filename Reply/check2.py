...
for i in trange(length):
    ...
    with h5py.File(filename) as h:
        ...
        diff = np.diff(time)
        index = (diff>1*1e-6) & (diff<1000*1e-6)
...
for d in np.arange(0, length):
    with h5py.File('/mnt/eternity/raw/run00000%d/%d.h5' % (fileno, d)) as h:
        # print(h['Readout']['TriggerInfo'].dtype)
        TriggerNo = h['Readout']['TriggerInfo']['TriggerNo'][:]
        Sec = h['Readout']['TriggerInfo']['Sec'][:]
        NanoSec = h['Readout']['TriggerInfo']['NanoSec'][:]
    
    with h5py.File('/mnt/eternity/charge/run00000%d/%d.h5' % (fileno, d)) as h:
        TriggerNo_ = h['AnswerWF']['TriggerNo']
    ...
    with h5py.File('/mnt/eternity/recon/run00000%d/%d.h5' % (fileno, d)) as h:
        ...
        e[ids] = rc['E_sph_out'][ids]
        v = np.float64(np.vstack((x, y, z)).T)
    if len(x) == 0:
        continue
    index1 = (e[:-1]>0.73) & (e[:-1]<0.87) & (e[1:]>0.6) & (e[1:]<1) & (np.linalg.norm(v, axis=1)[:-1] < 0.3) & (np.linalg.norm(v, axis=1)[1:] < 0.3) & (np.diff(NanoSec) < 1000*1e-6)
    # index1 = (EE2f>0.73) & (EE2f<0.87) & (EE2s>0.6) & (EE2s<1) & ((time2s-time2f)>10*1e-6) & ((time2s-time2f)<1000*1e-6) & (rr2<0.3)
    a1, b1 = np.unique(TriggerNo[:-1][index1], return_index = True)
    a2, b2 = np.unique(TriggerNo[1:][index1], return_index = True)

    for ii, i in tqdm(enumerate(np.unique(TriggerNo_))):
        if np.isin(i, TriggerNo[:-1][index1]):
            idx = TriggerNo_ == i
            charge = Charge[idx]
            ...
            e_f = e[:-1][resort] * np.ones_like(corr)
            time_f.append(corr)
        ...
        if np.isin(i, TriggerNo[1:][index1]):
            ...
            corr = hit_time - probe - Dt[pmtId,-2]
            corr = corr - weighted_quantile(corr, 0.15, charge)
            e_s = e[1:][resort] * np.ones_like(corr)
            time_s.append(corr)
