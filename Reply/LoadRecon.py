...
Dt = np.loadtxt('/tarski/JNE/JinpingData/Jinping_1ton_Data/CalibData/TimeCalibData/PMTTimeCalib_Run257toRun262.txt')
...
# all cut
index1 = (EE2f<3.3) & (EE2s>0.6) & (EE2s<1) & ((time2s-time2f)>10*1e-6) & ((time2s-time2f)<1000*1e-6) & (rr2<0.3)
...
for d in np.arange(0, length):
    ...
    for i in tqdm(np.unique(TriggerNo)):
        if np.isin(i, trigger_fast_):
            ...
            pmtId = ChannelID[idx]
            resort = b1[a1 == i]
            v = np.vstack((xx2f[resort], yy2f[resort], zz2f[resort])).T
            r = np.linalg.norm(v)
            theta = np.sum((PMT[pmtId] * v), axis=1) / r / np.linalg.norm(PMT[pmtId], axis=1)
            ...
            corr = hit_time - probe
            corr = corr - np.quantile(corr, 0.1)
