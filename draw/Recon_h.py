...
with PdfPages('Recon_h.pdf') as pp:
    for prefix in ['/mnt/stage/douwei/Upgrade/0.90/recon/point', '/mnt/stage/douwei/JP_1t_paper/recon/point']:
        for axis in ['x','z']:
            ...
            m = ax.contourf(np.arange(0,650,10), np.arange(0,650,10), data.T, cmap='Greys', norm=LogNorm(), vmax=1000)
            ...
            if (prefix == '/mnt/stage/douwei/JP_1t_paper/recon/point') & (axis=='x'):
                plt.axvline(350, c='r', ls='--', lw=1)
            if (prefix == '/mnt/stage/douwei/JP_1t_paper/recon/point') & (axis=='z'):
                plt.axvline(100, c='r', ls='--', lw=1) 
    ...
    for prefix in ['/mnt/stage/douwei/Upgrade/0.90/root/point', '/mnt/stage/douwei/JP_1t_paper/root/point']:
        ...
        for axis in ['x','z']:
            ...
            fig = plt.figure(figsize=(3, 3))
            ax = fig.add_subplot(1,1,1)
            m = ax.contourf(np.arange(0, 650, 10), np.arange(0, 650, 10), Hdata.T, norm=LogNorm(), cmap='Greys', vmax=1000)
            # fig.colorbar(m)
            ax.plot([0, 650], [0, 650], 'r')
            ax.set(xlim = (0, 650), ylim = (0, 650))
            ax.set_xlabel('True $%s$/mm' % axis, fontsize=18)
