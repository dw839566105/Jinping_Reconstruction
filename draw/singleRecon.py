...
with PdfPages('SingleRecon_h.pdf') as pp:
    for prefix in ['/mnt/stage/douwei/Upgrade/0.90/recon/point',]:
        for axis in ['x']:
            path = '%s/%s/2' % (prefix, axis)
            data = []
            for radius in np.arange(0.00, 0.65, 0.01):
            ...
            ax.contourf(np.arange(0,650,10), np.arange(0,650,10), data.T, cmap='Greys', norm=LogNorm(), vmax=1000)
            ax.plot([0, 650], [0, 650], 'r')
            ax.set(xlim = (0, 650), ylim = (0, 650), xlabel = 'Truth $%s$/mm' % axis, ylabel = 'Recon $%s$/mm' % axis)
            plt.xticks(np.arange(0, 650, 200), ['%d' % i for i in np.arange(0,650,200)])
            plt.yticks(np.arange(0, 650, 200), ['%d' % i for i in np.arange(0,650,200)])
            pp.savefig(fig)
            
    for prefix in ['/mnt/stage/douwei/Upgrade/0.90/root/point',]:
        ...
        for axis in ['x','z']:
            ...
            for radius in tqdm(np.arange(0.00, 0.65, 0.01)):
            ...
            fig = plt.figure(figsize=(4.5, 4.5))
            ax = fig.add_subplot(1,1,1)
            ax.contourf(np.arange(0, 650, 10), np.arange(0, 650, 10), Hdata.T, norm=LogNorm(), cmap='Greys', vmax=1000)
            ax.plot([0, 650], [0, 650], 'r')
            ax.set(xlim = (0, 650), ylim = (0, 650), xlabel = 'Truth $%s$/mm' % axis, ylabel = 'Recon $%s$/mm' % axis)
            plt.xticks(np.arange(0, 650, 200), ['%d' % i for i in np.arange(0,650,200)])
            plt.yticks(np.arange(0, 650, 200), ['%d' % i for i in np.arange(0,650,200)])
