...
base = baseFileName[0:-5][:-5]

for radius in np.arange(0,0.65,0.01):
    for count in np.arange(30):
        if (count == 0):
            tTruth.Add(base + '%.3f.root' % radius)
        else:
            try:
                tTruth.Add(base + '%.3f_%d.root' % (radius, count))
                print(base + '%.3f_%d.root' % (radius, count))
            except:
                break
# Loop for event
cnt = 0
